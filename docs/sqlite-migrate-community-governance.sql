...
-- Baseline assumption:
-- This one-time migration targets the runtime schema in internal/firefly/schema.sql.
-- It is not idempotent and should be executed only once on a database that does not yet
-- contain the community governance columns/tables below.

-- Snapshot original timestamps before any UPDATE fires trg_markers_updated_at.
...
-- Step 1: add consensus columns to markers
...
-- Step 2: backfill temporal fields
UPDATE markers
...
-- Step 4: backfill basic score defaults from existing visibility
UPDATE markers
SET support_score = CASE
    WHEN consensus_status = 'verified' THEN 8
    WHEN consensus_status = 'limited' THEN 3
    WHEN consensus_status = 'disputed' THEN 0
    ELSE 0
END,
dispute_score = CASE
...
-- Step 5: backfill expiration windows
UPDATE markers
SET expires_at = CASE
...
-- Step 6: create community feedback table
...
-- Step 7: create reputation profile table
...
-- Step 8: seed reputation profiles from existing marker authors
...
-- Step 9: create reputation event log
...
-- Step 10: backfill one reputation event per existing marker owner
...
-- Step 11: create escalation table
